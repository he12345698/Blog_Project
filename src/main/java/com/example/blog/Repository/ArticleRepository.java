...
public interface ArticleRepository extends JpaRepository<ArticleVo, Long> {
    ...
    @Query("SELECT a FROM ArticleVo a WHERE a.title LIKE %:title% AND a.authorId = :authorId")
    Page<ArticleVo> searchByTitleAndAuthor(@Param("title") String title, @Param("authorId") Long authorId, Pageable pageable);
